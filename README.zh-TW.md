...
安裝時若偵測到 Codex 或 Claude Code 等宿主可註冊互動審批適配器，安裝計畫會顯示該動作並預設開啟。Codex 使用對話內結構化審批卡；其他宿主只在原生 MCP 表單能保持可審閱佈局時使用原生面板。不支援時回退到綁定提案 ID 的文字確認。可用 `--no-interactive-approval` 退出。
...
V1.2.3 修復原生確認視窗不渲染 Markdown 時的可讀性問題。審批摘要改為主機無關的結構化純文字，使用【本次計畫】、【檔案變更】、【允許寫入範圍】與【審批邊界】穩定分區；即使主機折疊換行，仍能保留清晰的資訊邊界。完整 ID 與校驗資訊保留在審批記錄中，確認視窗聚焦動作、目標路徑、風險與寫入範圍。

V1.2.4 嘗試用 Unicode 強制斷行符改善 Codex 原生 MCP 表單的換行，但宿主元件仍可能正規化這些字元，因此不能把它當成穩定的跨版本顯示保證。

V1.2.5 不再把 Codex 的審批可讀性寄託在宿主原生表單如何處理空白字元上。Codex 桌面端優先顯示對話內結構化審批卡，以摘要卡、檔案表格、允許寫入範圍及「確認並繼續 / 要求修改 / 拒絕 / 暫不處理」按鈕完成選擇。按鈕只回傳綁定提案 ID 與完整計畫摘要的決策訊息；執行階段重新核驗後才會寫入，其他 Agent 仍使用原生 MCP 審批或精確文字確認。

V1.2.6 修復審批卡生成腳本中的換行跳脫錯誤，並新增生成後 JavaScript 語法解析測試，避免無效腳本進入對話渲染階段。

V1.2.7 明確停止在 Codex 原生 MCP 表單中承載結構化審批正文：Codex 改用對話內審批卡展示計畫、檔案、範圍與風險，MCP 只負責不可變提案、狀態、摘要校驗和確定性寫入。若誤調用原生審批工具，系統只返回審批卡接力資訊，不再彈出難以審閱的表單。
...
- 目前穩定版為 v1.3.2；軟體採用 AGPL-3.0-or-later，原創說明文件採用 CC BY-SA 4.0，並提供商業授權途徑；
...
- 支援「舊目錄唯讀 + 複製到新 Personal OS」的審閱式遷移，以及 Codex 對話內審批卡或相容宿主原生審批面板；不提供預設原地整理、獨立完整 GUI、雲端同步、向量資料庫、永久刪除或自動外部操作。
